...
export default function ChargeMap({ userId }) {
  ...
  const backF  = fatigueOf("BACK", "LOWER_BACK");
  ...
  if (loading) return (
    <div className="volt-screen">
      <div style={{ fontFamily:"'DM Sans', sans-serif", fontSize:13, color:C.muted, paddingTop:40, textAlign:"center" }}>Carregando charge map...</div>
    </div>
  );
    ...
    <div className="volt-screen">
      ...
      <div style={{ background:C.g2, borderRadius:18, padding:20, marginBottom:14, border:"1px solid rgba(255,210,0,0.06)", display:"flex", flexDirection:"column", alignItems:"center", gap:14 }}>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", width:"100%" }}>
          ...
          <span style={{ fontFamily:"'Space Mono', monospace", fontSize:9, background:"rgba(255,210,0,0.1)", color:C.yellow, padding:"3px 8px", borderRadius:6 }}>● Ativo</span>
          <span style={{ fontFamily:"'Space Mono', monospace", fontSize:9, background:"rgba(255,255,255,0.05)", color:C.muted, padding:"3px 8px", borderRadius:6 }}>● Descansado</span>
        </div>
      </div>
      ...
      <div style={{ display:"flex", gap:10, marginBottom:14 }}>
        <div style={{ flex:1, background:C.g2, borderRadius:14, padding:"14px 16px", border:"1px solid rgba(255,210,0,0.06)" }}>
          ...
          <div style={{ fontFamily:"'DM Sans', sans-serif", fontSize:11, color:C.muted, marginTop:4 }}>grupos ativos</div>
        </div>
        <div style={{ flex:1, background:C.g2, borderRadius:14, padding:"14px 16px", border:"1px solid rgba(255,210,0,0.06)" }}>
      ...
      <div style={{ background:C.graphite, borderRadius:12, padding:"12px 14px", border:"1px solid rgba(255,210,0,0.06)" }}>
        <span style={{ fontFamily:"'DM Sans', sans-serif", fontSize:12, color:C.text, lineHeight:1.6 }}>
          ...
          <span style={{ color:C.muted }}>
            {emRecarga > 0
              ? `${muscles.filter(m => m.recovering).map(m => m.name).join(", ")} em recuperação. Treine os grupos descansados. ⚡`
